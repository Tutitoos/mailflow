...
Current producers cover HTTP requests and latency, queue outcomes, Google synchronization pages, mail actions, CDN cleanup, Sentry envelope ingestion, and Go heap/goroutine gauges. Later backup and provider implementations must use the same registry rather than create a second telemetry path.
