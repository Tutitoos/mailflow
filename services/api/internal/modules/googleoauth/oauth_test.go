...
func TestOAuthRequiresPKCEAndSingleUseState(t *testing.T) {
	...
	account, err := service.Callback(context.Background(), state, "code")
	if err != nil || account.RemoteID != "google-subject" || accountStore.connected.Provider != accounts.ProviderGoogle {
