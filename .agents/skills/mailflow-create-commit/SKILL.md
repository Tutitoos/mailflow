...
1. Confirm the branch is not `main` and inspect the full repository status, including untracked files.
2. Separate task changes from pre-existing user changes. Stage explicit paths; never use broad staging when unrelated files exist.
...
7. Verify the new commit and remaining worktree. Report the SHA, subject, validation, and any changes deliberately left uncommitted.
