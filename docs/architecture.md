...
El shell macOS carga la misma SPA desde un único origen de instalación fijado al compilar. Ese origen remoto conserva cookies, REST y WebSocket de primera parte. Rust limita la navegación al origen exacto, abre destinos externos en el navegador del sistema y convierte únicamente callbacks `mailflow://` con rutas y valores permitidos en navegación interna. Sus capacidades IPC se limitan a la caché cifrada, la sesión nativa y comandos acotados para notificaciones, badge, updater y experiencia macOS; cada operación vuelve a validar datos y nunca acepta rutas de archivos ni SQL. Las notificaciones ocultan contenido por defecto, deduplican eventos y entregan únicamente identificadores validados a la SPA autenticada. El updater exige HTTPS, firma minisign, canal, target, bundle e identidad de release coincidentes antes de reemplazar la aplicación. La distribución macOS usa un único artefacto universal, firma Developer ID Application, hardened runtime, notarización y tickets grapados verificados antes de crear una release en borrador. Véanse [`desktop.md`](desktop.md), [`desktop-updater.md`](desktop-updater.md), [`macos-release.md`](macos-release.md), [`native-sessions.md`](native-sessions.md) y [`offline-cache.md`](offline-cache.md).
